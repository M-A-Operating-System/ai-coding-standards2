...
def _install_agents_symlink(
    ...
    if dst_dir.is_symlink():
        ...
        if not force:
            ...
            return 0
        if dry_run:
            print(f"  WOULD  {dst_dir} → {rel_target}  (replace directory with symlink)")
            return 1
...
def add_gitignore_entries(
    ...
    src_standards = SUBMODULE_ROOT / "standards"
    if src_standards.is_dir():
        for f in sorted(src_standards.glob("*.json")):
            if f.name.endswith(".schema.json") or f.name == "adrs.json":
                continue
            patterns.append(f"standards/{f.name}")
    ...
    needs_header = header not in existing_text
    block = (
...
def untrack_managed_paths(consuming_root: Path, dry_run: bool) -> int:
    ...
    if src_standards.is_dir():
        for f in sorted(src_standards.glob("*.json")):
            if f.name.endswith(".schema.json") or f.name == "adrs.json":
                continue
            candidates.append(f"standards/{f.name}")
...
def print_followup(consuming_root: Path) -> None:
    ...
    print()
    print(f"  2. Commit the new files (and any staged deletions from un-tracking):")
    ...
    print()
    print(f"     .claude/agents/, .claude/commands/, base standards/*.json,")
    print(f"     and .claude/settings.local.json are gitignored — managed by")
    print(f"     the daily sync-claude.yml workflow. On Linux they are")
    print(f"     symlinked; on Windows they are copied. If any of these were")
    print(f"     previously committed, get_started.py has already staged their")
    print(f"     removal via 'git rm --cached' (local files are untouched).")
    print()
    print(f"     PROJECT STANDARDS: standards/adrs.json is project-owned and")
    print(f"     IS committed. Add project-specific standards as NEW files")
    print(f"     (e.g. standards/myapp.json) — the daily sync never deletes")
    print(f"     your additions.")
    print()
    ...
    print(f"     The .claude/agents/ symlink/copies are for interactive Claude")
    print(f"     Code sessions only. Editing them does NOT affect pipeline")
    print(f"     execution. To change agent behaviour, pin to a fork or raise")
    print(f"     a PR upstream.")
    print()
