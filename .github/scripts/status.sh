...
# Bootstrap: create one-off trigger and human-gate labels that are not
# generated by bootstrap-all (which only creates {agent}:{status} labels).
status_bootstrap_trigger_labels() {
  local repo="${1:-$(_repo)}"

  # Trigger labels — applied by humans to start a pipeline phase
  _ensure_label "$repo" "build:requested"          "0E8A16" "Trigger the coder to implement an approved PRD"
  _ensure_label "$repo" "pr-review:requested"      "0075CA" "Request a structured PR review with merge recommendation"
  _ensure_label "$repo" "codebase-review:requested" "7057FF" "Trigger an ad-hoc three-persona codebase review"

  # Human-gate labels — applied by humans to advance the pipeline past a gate
  _ensure_label "$repo" "prd:approved"   "D4C5F9" "PRD approved by stakeholder; advance pipeline"
  _ensure_label "$repo" "pr:approved"    "D4C5F9" "PR approved by reviewer; advance pipeline"
...
Commands:
  ...
  bootstrap-triggers                              Create trigger and human-gate labels (build:requested, pr-review:requested, etc.)
