    ...
    for agent_def in agents:
            ...
            while not dry_run:
                ...
                try:
                    gh.post_comment(
                        ...
                        f"exited non-zero without a status sentinel; retrying after {_backoff}s.",
                    )
                except Exception as _retry_comment_exc:
                    log.debug("  could not post retry comment: %s", _retry_comment_exc)
                time.sleep(_backoff)
                result = invoke_agent(
        ...
        if not dry_run:
            ...
            if sentinel_status:
                ...
                _exhaustion_reason = (
                    f"_Retry limit of {agent_def.max_retries} exhausted after "
