...
def _resolve_agent_invocation(
    ...
    *,
    ...
    prompt = (
        ...
        f"AI_AGILE_SCRATCH={_scratch_path(agent_session_id).strip()}\n\n"
        f"Before exiting, write your result to {_scratch_path(agent_session_id).strip()}/{_RESULT_FILENAME} "
        f"(the resolved AI_AGILE_SCRATCH path above — use the Write tool with this literal path; "
        f"see AGENTS.md's \"How you communicate\" for why). "
