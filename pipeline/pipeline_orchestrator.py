...
def _untracked_root_files() -> set:
    ...
    relative path instead, the file lands here. Returns an empty set if git
    is unavailable -- this is a hygiene sweep and must never fail a run.
    """
    try:
        out = subprocess.run(
...
def _sweep_agent_root_files(agent_def: AgentDef, before: set) -> None:
    ...
    """
    leaked = sorted(_untracked_root_files() - before)
    if not leaked:
        return
    log.warning(
