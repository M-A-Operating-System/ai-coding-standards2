...
| A scheduled flow that only reviews | No scheduled flow exists to constrain. The nearest analogues are the on-demand cleanup agents, which do follow the shape: `branch-cleanup` and `issue-cleanup` post a recommendation and delete or close nothing until a human names what to act on. Nothing declares the constraint, so nothing enforces it |
| Provenance on a raised issue | Designed but not implemented. `04-lifecycle.md` specifies `Gap-source:` and `Debt-source:` trailers; neither appears anywhere in the code or the agent prompts. They are also two names for one concept -- which flow produced this issue -- and one trailer would serve every scheduled flow rather than a new name per loop |
| Reporting only what is new | Does not exist anywhere. No agent reads its own previous output before reporting, so a flow that ran weekly would raise the same findings every week. `branch-cleanup` and `issue-cleanup` re-evaluate from scratch on each invocation, which is correct for an on-demand sweep and would be wrong for a scheduled one |
