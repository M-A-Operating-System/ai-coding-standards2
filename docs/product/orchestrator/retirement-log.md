...
| Conformance and traceability | issue #393 (the build plan) | `gap_analysis.md` retired; its per-promise VIOLATED/PARTIAL/UNTESTED audit is not migrated -- superseded by the sequenced build plan on issue #393, which tracks closing the gap rather than cataloguing it. The individual defects it cited (#335, #356, #373, #377, #380, #383, #388, and others) remain the record of specific current-state evidence |
...
| Human gates, which ones exist today | [`lifecycle.md`](lifecycle.md#human-gates) | current -- `07-human-gates.md` retired |
...
| Human interaction -- markers, gates, agent identity | `PRODUCT.md` (the step contract's marker table, MI-7) | draft -- `09-human-interaction.md` retired; nothing migrated. Agent identity duplicated MI-7's existing 'How a non-human actor is recognised'; agent announcements restated the step contract's existing return/marker requirements at a level of detail nothing needs; Question Cards were never carried into the target design -- a `question` marker existed in the now-retired `09-human-interaction.md` and the agent template, but no spawned step can ask and wait, so `blocked` is the channel and headless steps have it, interactive sessions are a conversation, neither needs a bespoke Q&A protocol |
