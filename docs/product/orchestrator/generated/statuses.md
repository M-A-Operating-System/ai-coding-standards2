...
| `:complete` | Agent completed its work successfully. All automated checks passed. If the agent has a human_gate_label configured, the pipeline still waits for that gate before the next agent fires. A :complete records the subject its conclusion was reached against. When that subject is no longer the current one the record is superseded, not wrong, and the step runs again and replaces it -- the step correcting itself, which needs no permission. A record naming no subject is never treated as superseded. | yes | no | no | the owning step, when the subject its record named is no longer the current one; otherwise a human correcting a record that is wrong |
...
| `:exhausted` | Agent ran out of its turn or wall-clock budget before it could write a result. Distinct from failed: the agent did not crash or return something malformed, it simply did not finish inside the budget it was given. The orchestrator never retries an exhausted run automatically -- the budget itself has not changed between runs, so an identical retry would hit the same wall. | yes | yes | yes | human |
| `:exhausted-partial` | Companion label applied alongside exhausted, never on its own: the run's isolated worktree held uncommitted or unpushed changes when the budget ran out, and the orchestrator pushed them to the branch before the worktree was torn down (issue #445) rather than discarding them. Unreviewed -- the run never reached its own self-review step or wrote a result -- so this marks recovered, not vetted, work. | no | no | no | orchestrator, superseded on the run's next exhaustion (removed first, then re-applied only if that run also has something to salvage) -- otherwise stays until a human clears exhausted and the retry lands a normal outcome |
...
| `:complete` | orchestrator | Treat as a satisfied dependency for downstream agents. Before skipping a completed step, compare the subject its last closing record named against the current one; re-run the step when both are established and they differ. Not being able to establish either is not evidence of staleness -- leave the record alone. |
...
| `:exhausted` | orchestrator | Halt pipeline for this item. Do not retry automatically -- require a human to remove the label. |
| `:exhausted-partial` | orchestrator | Informational only; exhausted is what halts the pipeline and requires human action. Never applied by a step's own allowed_labels -- only the orchestrator's exhaustion handling sets or clears it. |
