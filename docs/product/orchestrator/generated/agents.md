...
Reviews the draft PR for this issue after CI passes. Looks up the open PR by branch issue-{N}, reads the diff and linked spec, then reports findings covering correctness, design alignment, standards compliance, and security -- the orchestrator computes the verdict from those findings (outcome_policy: review_findings), never trusting the model's own outcome field. A Critical finding always blocks; a non-Critical finding below 0.8 confidence, a non-Critical improvement-category finding, or one covered by a verified ADR exception does not. A Low-severity finding blocks unless its complexity is medium (flagged for a human decision, not auto-blocked) or high (deferred: the orchestrator bundles it into a follow-up GitHub issue, STD-ARCH-007) -- issue #506. Cannot APPROVE when any unresolved human REQUEST_CHANGES reviews exist on the PR -- this is a hard block regardless of automated findings. On APPROVE, the orchestrator compares the reviewed head_sha with the PR's live head (require_head_match, issue #512 Part 3); a mismatch withholds ready-marking and re-dispatches this step on the next tick rather than approving a commit that was never reviewed. On APPROVE with a matching head and no unresolved human reviews, the orchestrator marks the issue PR ready for human review. On REQUEST CHANGES the orchestrator automatically re-invokes the coder (clearing ci-gate:complete so CI re-runs, up to 3 cycles); only after 3 failed cycles is human sign-off required. Skipped for epic/blocked issues.
