...
## Scenario: A leaked root file is reported rather than passing silently

**Given** a step commits a working file at the repo root instead of writing it into `$AI_AGILE_SCRATCH`
**When** the orchestrator pushes the step's branch
**Then** the step is reported failed with the offending path named
**And** the commits are pushed rather than discarded, because the rest of the work is real

## Scenario: The root-file guard does not cost the step its real work

**Given** a step commits a change to a tracked root file such as `README.md` alongside a new nested file
**When** the orchestrator pushes the step's branch
**Then** both land and nothing is reported -- only *new* files at depth 0 are refused

## Scenario: A run that commits nothing moves no branch

**Given** a step that committed nothing and left nothing uncommitted
**When** the orchestrator reaches the push
**Then** no push is attempted and the step is not reported failed
