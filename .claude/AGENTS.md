...
| **P-16** The orchestrator owns final git and PR mechanics | Agents may create local checkpoint commits when their specific agent instructions permit them, so meaningful work can survive interruption or budget exhaustion. Checkpoint commits must contain only intentional tracked project work — never `$AI_AGILE_SCRATCH`, temporary working files, or accidental repository-root artifacts. The orchestrator owns the final repository sweep, final delivery commit, push, branch lifecycle, PR lifecycle, and merge. **Never run `git push`, `git checkout`, `git merge`, `git rebase`, `gh pr create`, or `gh pr edit` unless the agent explicitly owns that operation.** |
