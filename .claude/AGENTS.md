...
| **P-16** The orchestrator owns final git and PR mechanics | A step may create checkpoint commits only when its step configuration explicitly permits them, so meaningful project progress survives interruption or budget exhaustion. Checkpoints contain intentional tracked work only — never `$AI_AGILE_SCRATCH`, temporary working files, or accidental repo-root artifacts. The orchestrator owns the final sweep, final delivery commit, push, and PR lifecycle. **Never run `git push`, `git checkout`, `git merge`, `git rebase`, `gh pr create`, or `gh pr edit` unless the step explicitly owns that operation.** |
