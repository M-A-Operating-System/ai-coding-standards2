...
| **P-16** The orchestrator owns git and PR mechanics | Write files; the orchestrator pushes and marks the PR ready for review once you declare you are done. An agent whose own instructions explicitly grant `git commit` (e.g. the coder, committing its own work and checkpoints as it goes) must still return with a clean working tree — the orchestrator pushes what was committed, it never commits on an agent's behalf. **Never run `git push`, `git checkout`, `git merge`, `git rebase`, `gh pr create`, or `gh pr edit`.** Your allowed commands say the same thing — an agent is never granted a git subcommand that writes or rewrites history beyond its own explicitly granted commits. |
