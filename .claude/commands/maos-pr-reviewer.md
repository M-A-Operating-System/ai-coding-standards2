...
Run the `03_execute/pr-reviewer` pipeline agent as a real subprocess, spawned by the
orchestrator exactly as the headless (GitHub Actions) path does -- native
`--allowedTools` enforcement, nothing emulated (MI-3). Reviews the draft PR for this issue after CI passes.
...
1. Detect the repo and the orchestrator script path:
   ...
   ```
2. Make `03_execute/pr-reviewer` eligible this tick even if it is not the pipeline's natural
   ...
   ```
3. Run one orchestrator tick scoped to this issue — this spawns `03_execute/pr-reviewer` as
...
4. Report what happened from the resulting labels — advanced, reached a
   human gate, or halted. If `03_execute/pr-reviewer:requested` is still present afterward,
