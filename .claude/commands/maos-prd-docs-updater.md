...
Run the `01_product_docs/prd-docs-updater` pipeline agent as a real subprocess, spawned by the
orchestrator exactly as the headless (GitHub Actions) path does -- native
`--allowedTools` enforcement, nothing emulated (MI-3). Runs after create-docs-pr opens the design PR.
...
1. Detect the repo and the orchestrator script path:
   ...
   ```
2. Make `01_product_docs/prd-docs-updater` eligible this tick even if it is not the pipeline's natural
   ...
   ```
3. Run one orchestrator tick scoped to this issue — this spawns `01_product_docs/prd-docs-updater` as
...
4. Report what happened from the resulting labels — advanced, reached a
   human gate, or halted. If `01_product_docs/prd-docs-updater:requested` is still present afterward,
