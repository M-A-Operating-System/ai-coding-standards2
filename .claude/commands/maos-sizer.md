...
Run the `00_ondemand/sizer` pipeline agent as a real subprocess, spawned by the
orchestrator exactly as the headless (GitHub Actions) path does -- native
`--allowedTools` enforcement, nothing emulated (MI-3). Ad-hoc issue sizer.
...
1. Detect the repo and the orchestrator script path:
   ...
   ```
2. Make `00_ondemand/sizer` eligible this tick even if it is not the pipeline's natural
