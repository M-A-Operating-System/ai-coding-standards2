...
`--allowedTools` enforcement, nothing emulated (MI-3). Implements one GitHub issue in the consuming repository.
...
1. Detect the repo and the orchestrator script path:
   ...
   ```
2. Make `03_execute/coder` eligible this tick even if it is not the pipeline's natural
   ...
   ```
3. Run one orchestrator tick scoped to this issue — this spawns `03_execute/coder` as
...
4. Report what happened from the resulting labels — advanced, reached a
   human gate, or halted. If `coder:requested` is still present afterward,
