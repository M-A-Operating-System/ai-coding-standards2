...
Run the `00_ondemand/issue-cleanup` pipeline agent as a real subprocess, spawned by the
orchestrator exactly as the headless (GitHub Actions) path does -- native
`--allowedTools` enforcement, nothing emulated (MI-3). Ad-hoc backlog-hygiene sweep.
...
1. Detect the repo and the orchestrator script path:
   ...
   ```
2. Make `00_ondemand/issue-cleanup` eligible this tick even if it is not the pipeline's natural
   ...
   ```
3. Run one orchestrator tick scoped to this issue — this spawns `00_ondemand/issue-cleanup` as
...
4. Report what happened from the resulting labels — advanced, reached a
   human gate, or halted. If `00_ondemand/issue-cleanup:requested` is still present afterward,
